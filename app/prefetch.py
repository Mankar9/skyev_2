# https://dfir-scripts.medium.com/extracting-windows-prefetch-files-41a6aac63291
...
def parse_file(prefetch_file,outpath):
    try:
        ...
        strings_file = open(stringsfile, 'a+', newline='')
        with strings_file:
        ...
        for exe_timestamp in range(8):
            if scca.get_last_run_time_as_integer(exe_timestamp) > 0:
                ...
                if int(prefetch_version) <30:
                    run_count_file = open(runcountsfile, 'a+', newline='')
                    with run_count_file:
...
def main():
    ...
    pf_header='last_run_time,exe_file,pf_hash,pf_run_count,pf_file,pf_version,volume_count,volume_timestamp,volume_dev_path,volume_serial_number'
